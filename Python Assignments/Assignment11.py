...
def registration_form():
    ...
    statechoosen['values'] = (
    'Maharastra', 'Karnataka', "Andra Pradesh", "West Bangal", "Bihar", "Meghalaya", "Uttar Pradesh")
    statechoosen.current()
    statechoosen.place(x=85, y=241)

    Label(registration_screen, text=" ", textvariable=message).place(x=95, y=260)

    Button(registration_screen, text="Register", width=10, height=1, bg="orange", command=registration).place(x=110,
                                                                                                              y=301)
